...
const /**
     ...
     */
    timerFunctionNames = ['Timeout', 'Interval', 'Immediate', 'Event', 'Promise'],
    ...
    /**
     * This object defines a set of timer function names that are only markers for holding time.
     * Such timers, when placed in generic rules, needs special attention.
     *
     * @constant {Array.<Boolean>}
     */
    markerTimerFunctions = {
        Promise: true
    },
...
function Timerz (delegations, onError, onAnyTimerStart, onAllTimerEnd) {
    ...
    (typeof timers.setPromise !== FUNCTION) && (function () {
        var promises = {},
            total = 0;

        timers.setPromise = function (computeTimers) {
            var id = ++total;

            promises[id] = computeTimers;

            return id;
        };

        timers.clearPromise = function (id) {
            const cb = promises[id];

            delete promises[id];
            (typeof cb === FUNCTION) && cb.apply(dummyContext, arrayProtoSlice.call(arguments, 1));
        };

        timers.clearAllPromises = function () {
            Object.keys(promises).forEach(function (prop) {
                delete promises[prop];
            });
        };
    }());
    ...
    computeTimerEvents = function (increment, clearing) {
        ...
        // queue to avoid a race condition with new async resources
        // most applicable to Promises
        setImmediate(() => {
    ...
    timerFunctionNames.forEach(function (name) {
        ...
        this[('set' + name)] = function (callback) {
            ...
            if (sealed || (typeof callback !== FUNCTION && !markerTimerFunctions[name])) {
                return;
            }
     ...
     * @returns {Boolean}
     */
    this.didExecuteAsynchronously = function () {
        return wentAsync;
    };
    ...
    this.terminate = function () {
        ...
        onAllTimerEnd.apply(dummyContext, arguments);
    };
}
